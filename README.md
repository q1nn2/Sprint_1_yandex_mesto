...
- **Проектирование и ведение тест-кейсов (включая Blocked‑сценарии)**
- **Оформление баг-репортов и ретеста**
- **Оценку качества продукта и рекомендации по релизу**
...
- **testcases/** — тест-кейсы по ключевым пользовательским сценариям:
  - редактирование профиля (имя, занятие),
  - работа с карточками,
  - валидация полей и кнопки «Сохранить».
- **bugreports/** — баг-репорты с шагами воспроизведения, ER/AR, окружением и приоритетом.
- **retest/** — результаты ретеста дефектов (какие баги закрыты, какие переоткрыты).
...
Для ключевых требований к редактированию профиля и валидации полей построена простая матрица трассируемости (пример, подстрой под свою нумерацию при необходимости):

| Требование                           | Тест-кейсы           | Найденные баги                    |
|--------------------------------------|----------------------|-----------------------------------|
| RQ-1: Редактирование профиля         | TK-02, TK-03, TK-04  | SP1_TK2, SP1_TK3, SP1_TK4         |
| RQ-2: Валидация полей при сохранении | TK-09, TK-17         | SP1_TK2                           |

Каждый тест-кейс оформлен по единому шаблону:

- **ID** (например, `TK-02`)
- **Название** (кратко отражает цель проверки)
- **Предусловия**
- **Шаги**
- **Ожидаемый результат**
...
Единый формат облегчает поддержку набора тестов и планирование регрессии.
